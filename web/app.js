...
// ---------- dropdown menu helper (design B: header "Ещё" + per-card kebab) ----------
let OPEN_MENU = null; // { trigger, list }
function closeOpenMenu() {
  if (!OPEN_MENU) return;
  OPEN_MENU.list.hidden = true;
  OPEN_MENU.trigger.setAttribute('aria-expanded', 'false');
  OPEN_MENU = null;
}
function toggleMenu(trigger, list) {
  if (OPEN_MENU && OPEN_MENU.list === list) { closeOpenMenu(); return; }
  closeOpenMenu();
  list.hidden = false;
  trigger.setAttribute('aria-expanded', 'true');
  OPEN_MENU = { trigger, list };
}
document.addEventListener('click', e => {
  if (!OPEN_MENU) return;
  if (OPEN_MENU.list.contains(e.target) || OPEN_MENU.trigger.contains(e.target)) return;
  closeOpenMenu();
});
document.addEventListener('keydown', e => { if (e.key === 'Escape' && OPEN_MENU) closeOpenMenu(); });
...
function renderDashboard(filter) {
  ...
  grid.innerHTML = idx.map(p => {
    ...
    return `<div class="card" data-open="${p.id}">
      <button class="kebab" data-kebab="${p.id}" type="button" aria-haspopup="true" aria-expanded="false" aria-label="Действия со щитом «${esc(p.name)}»" title="Действия со щитом">⋯</button>
      ...
      <div class="cact" hidden role="menu">
        <button data-rename="${p.id}" class="menu-item">Переименовать</button>
        <button data-dup="${p.id}" class="menu-item">Дублировать</button>
        <button data-export="${p.id}" class="menu-item" title="Сохранить файл проекта (.ecproj.json)">Файл проекта</button>
        <button data-del="${p.id}" class="menu-item">Удалить</button>
      </div></div>`;
  }).join('');
}
...
$('projectGrid').addEventListener('click', e => {
  ...
  if (d.kebab) { e.stopPropagation(); toggleMenu(t, t.closest('.card').querySelector('.cact')); return; }
  closeOpenMenu();
...
$('btnMore').addEventListener('click', e => { e.stopPropagation(); toggleMenu($('btnMore'), $('boardMoreList')); });
$('boardMoreList').addEventListener('click', () => closeOpenMenu());
...
// ---------- charts (Plotly/SVG) ----------
const BASE = { paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)', font: { color: '#55606E', size: 12, family: "'Inter', -apple-system, sans-serif" }, margin: { l: 60, r: 18, t: 46, b: 70 }, legend: { orientation: 'h', y: -0.22, yanchor: 'top', x: 0 }, showlegend: true };
...
function renderTCC(t) {
  ...
  if (!deviceOff) traces.push(
    { x: dmax.map(p => p[0]), y: dmax.map(p => p[1]), mode: 'lines', line: { color: '#1D4ED8', width: 1 }, showlegend: false },
    { x: dmin.map(p => p[0]), y: dmin.map(p => p[1]), name: `${t.device.class}${t.device.class === 'gG_fuse' ? '' : ' ' + t.device.curve_type} (полоса)`, mode: 'lines', line: { color: '#1D4ED8', width: 1 }, fill: 'tonexty', fillcolor: 'rgba(29,78,216,.10)' },
  );
  const shapes = [], anns = [];
  vlines(shapes, t.markers.IB, '#8A94A3', 'dot', 'IB', anns); vlines(shapes, t.markers.In, '#55606E', 'dash', 'In', anns); vlines(shapes, t.markers.Iscc, '#DC2626', 'dot', 'Iscc', anns);
  if (deviceOff) anns.push({ xref: 'paper', yref: 'paper', x: 0.5, y: 0.5, text: 'кривая аппарата отсутствует в норм-пакете', showarrow: false, font: { color: '#8A94A3', size: 12 } });
  const coord = deviceOff ? '' : (t.coordinated === null ? '' : (t.coordinated ? '  ·  иллюстративная проверка: OK' : '  ·  не координируется (иллюстр.)'));
  Plotly.react('plot_tcc', traces, Object.assign({}, BASE, { title: { text: 'Время-токовая координация' + coord, font: { size: 14, color: t.coordinated === false && !deviceOff ? '#DC2626' : '#55606E' } }, xaxis: { type: 'log', title: 'Ток, A', gridcolor: '#EEF0F3' }, yaxis: { type: 'log', title: 'Время, с', gridcolor: '#EEF0F3' }, shapes, annotations: anns }), CFG);
}
function renderSweep(s) {
  if (!s) return; const x = s.rows.map(r => fmt(r.section_mm2)), y = s.rows.map(r => r.Iz_a);
  const colors = s.rows.map(r => r.section_mm2 === s.chosen_mm2 ? '#1D4ED8' : (r.amp_ok && r.vd_ok && r.sc_ok ? '#17803D' : '#C7CDD6'));
  Plotly.react('plot_sweep', [{ x, y, type: 'bar', marker: { color: colors } }], Object.assign({}, BASE, { title: { text: `Подбор сечения — выбрано ${fmt(s.chosen_mm2)} мм² (${govLabel(s.governing)})`, font: { size: 14 } }, xaxis: { title: 'Сечение, мм²', type: 'category' }, yaxis: { title: 'IZ, A', gridcolor: '#EEF0F3' }, showlegend: false, shapes: [{ type: 'line', xref: 'paper', x0: 0, x1: 1, y0: s.required_iz_a, y1: s.required_iz_a, line: { color: '#55606E', width: 1.5, dash: 'dash' } }], annotations: [{ xref: 'paper', x: 0.01, y: s.required_iz_a, text: `требуемый IZ ≥ ${fmt(s.required_iz_a)} A`, showarrow: false, font: { color: '#55606E', size: 11 }, yanchor: 'bottom' }] }), CFG);
}
function renderVD(v) {
  if (!v) return;
  Plotly.react('plot_vd', [{ x: v.series.map(p => p[0]), y: v.series.map(p => p[1]), mode: 'lines', name: `ΔU при ${fmt(v.section_mm2)} мм²`, line: { color: '#1D4ED8', width: 2.5 } }, { x: [v.current_length_m], y: [v.current_pct], mode: 'markers', name: 'текущая длина', marker: { color: '#111418', size: 9 } }], Object.assign({}, BASE, { title: { text: 'Профиль падения напряжения', font: { size: 14 } }, xaxis: { title: 'Длина, м', gridcolor: '#EEF0F3' }, yaxis: { title: 'ΔU, %', gridcolor: '#EEF0F3' }, shapes: [{ type: 'line', xref: 'paper', x0: 0, x1: 1, y0: v.limit_pct, y1: v.limit_pct, line: { color: '#DC2626', width: 1.5, dash: 'dash' } }], annotations: [{ xref: 'paper', x: 0.01, y: v.limit_pct, text: `предел ${fmt(v.limit_pct)} %`, showarrow: false, font: { color: '#DC2626', size: 11 }, yanchor: 'bottom' }] }), CFG);
}
function renderDerating(d) {
  if (!d) return;
  Plotly.react('plot_derating', [{ x: d.stages.map(s => s.label), y: d.stages.map(s => s.value), type: 'bar', marker: { color: ['#8A94A3', '#55606E', '#1D4ED8'] }, text: d.stages.map(s => fmt(s.value)), textposition: 'outside' }], Object.assign({}, BASE, { title: { text: `Поправочные коэффициенты: It → IZ (нужно ≥ ${fmt(d.required_iz_a)} A)`, font: { size: 14 } }, yaxis: { title: 'A', gridcolor: '#EEF0F3' }, showlegend: false, shapes: [{ type: 'line', xref: 'paper', x0: 0, x1: 1, y0: d.required_iz_a, y1: d.required_iz_a, line: { color: '#55606E', width: 1.5, dash: 'dash' } }] }), CFG);
}
function renderSLD(s) {
  if (!s) return; const col = { PASS: '#17803D', FAIL: '#DC2626', NEEDS_REVIEW: '#B45309' }[s.status] || '#8A94A3';
  ...
  s.nodes.forEach((n, idx) => {
    if (idx > 0) svg += `<line x1="${x - gap}" y1="${y + h / 2}" x2="${x}" y2="${y + h / 2}" stroke="#111418" stroke-width="2"/>`;
    svg += `<rect x="${x}" y="${y}" width="${bw}" height="${h}" rx="9" fill="#FFFFFF" stroke="${idx === s.nodes.length - 1 ? col : '#111418'}" stroke-width="2"/>`;
    svg += `<text x="${x + bw / 2}" y="${y + 30}" fill="#111418" font-size="14" font-weight="600" text-anchor="middle">${esc(deviceText(n.label))}</text>`;
    svg += `<text x="${x + bw / 2}" y="${y + 52}" fill="#55606E" font-size="12" text-anchor="middle" font-family="monospace">${esc(deviceText(n.sub))}</text>`;
    x += bw + gap;
  });
