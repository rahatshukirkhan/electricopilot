...
> Версионированное расширение без изменения замороженных sizing-сигнатур: криптографическая
> идентичность project/report/normcheck определена в `docs/18-calculation-identity.md`.
...
class DataPack(BaseModel):
    ...
    def has_ampacity(self, method, material, insulation, n, size_mm2) -> bool: ...
        # True iff this exact section is in the ampacity table for the combo. Lets the sizer
        # sweep skip sections a real pack doesn't cover (docs/04 §4.6); does NOT gate
        # ambient/grouping off-table, which still raises inside corrected_ampacity.
