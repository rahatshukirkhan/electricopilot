...
получает проект, создаёт один свежий снимок `ProjectCalculationSnapshot` и вызывает
числовое ядро ровно по одному разу для каждой цепи. `build_project_report()` и правила
получают одни и те же `SizingResult` из этого снимка и возвращают проверяемые замечания. Поля
...
`ProjectCalculationSnapshot` и `project report`, построенного из того же набора
`SizingResult`. Снимок создаётся заново на каждый запрос: это не кэш между запросами и
не допускает сохранённый `circuit.result`, localStorage, импорт или Neon как источник
инженерных чисел. Реестр содержит ровно одну чистую функцию на правило. Правила не
вызывают LLM, сеть или часы и не мутируют проект/пак.
...
- регрессия инструментирует ядро: запрос нормоконтроля для N цепей вызывает `size()`
  ровно N раз и сохраняет findings неизменными;
